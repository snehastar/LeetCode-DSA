...
// check out favourite leetcode solution for understanding also striver's video on 3d dp
...
public:
    ...
    int f(int i1, int j1,int i2, int n, vector<vector<int>>& grid, vector<vector<vector<vector<int>>>> &dp){
        int j2 = i1+j1-i2;
        if(i1==n && j1==n){//base case
            return grid[i1][j1];//count only once
        }
        ...
        int dd = f(i1+1,j1,i2+1,n,grid,dp);
        int dr = f(i1+1,j1,i2,n,grid,dp);
        int rd = f(i1,j1+1,i2+1,n,grid,dp);
        int rr = f(i1,j1+1,i2,n,grid,dp);
        int maxi = max(max(dd,dr),max(rd,rr));
        return dp[i1][j1][i2][j2] = maxi+cherries;
    }
    ...
    int cherryPickup(vector<vector<int>>& grid) {
        ...
        vector<vector<int>> (n,
        vector<int>(n,-2))));   
        return max(maxi,f(0,0,0,n-1,grid,dp));
    }
};